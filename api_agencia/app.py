...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'Super_Secreto_Agencia_2026_No_Compartir'
CORS(app)
PORT = 3003
...
try:
    ...
    if not repo.obtener_usuario_por_email('admin@demo.com'):
        print("Creando admin...")
        repo.crear_usuario('admin@demo.com', generate_password_hash('admin123'), 'admin', 'Administración Central')
...
def register():
    ...
    if data is None:
        return jsonify({"error": "Flask no detectó un JSON válido. Verifica la caché de tu navegador."}), 400
    ...
    try:
        ...
        repo.crear_usuario(email, generate_password_hash(password), tipo, nombre_agencia)
...
def token_requerido(f):
    ...
    def decorador(*args, **kwargs):
        ...
        try:
            ...
            datos_usuario = jwt.decode(token, app.config['SECRET_KEY'], algorithms=["HS256"])
        ...
        if request.method in ['POST', 'PUT', 'DELETE']:
            ...
            if not csrf_token:
                return jsonify({'error': 'Ataque CSRF detectado: Falta token de seguridad'}), 403
...
def crear_reserva(datos_usuario): # El decorador nos pasa los datos del JWT
    ...
    try:
        ...
        if not url_destino: return jsonify({"error": "Tipo inválido"}), 400
        
        resp = requests.post(url_destino)
        if resp.status_code != 200: return jsonify({"error": "Sin stock"}), 409
...
@app.route('/api/agencia/reservas/<email>', methods=['GET'])
def mis_reservas(email):
    user = repo.obtener_usuario_por_email(email)
    if not user: return jsonify([]), 200
    
    reservas_raw = repo.obtener_reservas_usuario(user[0])
    
    # Mapeo de datos (DTO simple)
    reservas = [{"id": r[0], "tipo": r[1], "detalles": r[2], "fecha": str(r[3])} for r in reservas_raw]
...
def setup_2fa():
    ...
    totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(name=email, issuer_name="Agencia KIS")
